...
export default function Answer_c({ qnAnswers }) {
    ...
    const markCorrectOrNot = (qna, idx) => {
        if ([qna.answer, qna.selected].includes(idx)) {
            return { sx: { color: qna.answer === idx ? green[600] : red[500], fontWeight: 'bold' } };
        }
    };
    ...
    return (
        <Box sx={{ mt: 5, width: '100%', maxWidth: 700, mx: 'auto', p: 2 }}>
            {qnAnswers.map((item, j) => (
                ...
                >
                    <AccordionSummary
                        expandIcon={<ExpandCircleDownIcon sx={{ color: item.answer === item.selected ? green[500] : red[500] }} />}
                        sx={{
                    ...
                    >
                        <Typography sx={{ fontWeight: 'bold', fontSize: '1.1rem' }}>{item.qnInWords}</Typography>
                    </AccordionSummary>
                    <AccordionDetails sx={{ backgroundColor: 'white', borderRadius: '8px', p: 2 }}>
                        {item.imageName && (
                            <CardMedia
                                ...
                                sx={{ display: 'block', maxWidth: '100%', borderRadius: 2, mx: 'auto', my: 2 }}
                            />
                        )}
                        <List>
